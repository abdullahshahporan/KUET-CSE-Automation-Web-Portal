      ...
      <section className="max-w-6xl mx-auto px-6 md:px-8 py-16">
        ...
        {data.hodMessage && (
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="bg-white rounded-2xl overflow-hidden border border-gray-200 shadow-warm-lg mb-12">
            <div className="grid md:grid-cols-3 gap-0">
              <div className="relative overflow-hidden">
                <img src={getImageUrl(data.hodMessage.photo_path)} alt={data.hodMessage.name}
                  className="w-full h-64 md:h-full object-cover" />
                <div className="absolute inset-0 bg-gray-600/15" />
                <div className="absolute bottom-0 inset-x-0 h-1/3 bg-gradient-to-t from-[#161a1d]/50 to-transparent" />
              </div>
              <div className="md:col-span-2 p-8">
                <span className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-2 block">Head of Department</span>
                <h2 className="text-2xl font-bold text-gray-900 mb-1">{data.hodMessage.name}</h2>
                <p className="text-gray-400 mb-4">{data.hodMessage.designation}</p>
                <p className="text-gray-500 leading-relaxed italic">&ldquo;{data.hodMessage.message}&rdquo;</p>
              </div>
            </div>
          </motion.div>
        )}
        ...
        <div className="text-center py-16">
          ...
          <h3 className="text-xl font-bold text-gray-900 mb-2">Faculty Directory</h3>
          <p className="text-gray-500 max-w-md mx-auto">
            ...
            <a href="/auth/signin" className="text-gray-600 font-semibold hover:text-gray-400 underline">dashboard</a> after signing in.
          </p>
        </div>
      </section>
    </PublicLayout>
