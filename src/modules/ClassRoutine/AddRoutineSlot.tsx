...
export default function AddRoutineSlot({ show, onClose, onSave, term, session, section }: AddRoutineSlotProps) {
  ...
  // Styles
  const selectClass =
    'w-full px-3 py-2.5 mt-1 border border-gray-200 dark:border-[#3d4951] rounded-lg bg-white dark:bg-[#161a1d] text-gray-700 dark:text-white focus:ring-2 focus:ring-indigo-300 dark:focus:ring-red-400 focus:border-transparent transition-colors';
  const optionClass = 'bg-white dark:bg-[#161a1d] text-gray-700 dark:text-white';
      ...
      {show && (
        ...
        >
          <motion.div
            ...
            className="bg-white dark:bg-[#161a1d] rounded-2xl p-6 w-full max-w-lg border border-gray-200 dark:border-[#3d4951] shadow-2xl max-h-[90vh] overflow-y-auto"
          >
            {/* Header */}
            <div className="flex justify-between items-center mb-5">
              <h2 className="text-xl font-bold text-gray-700 dark:text-white">Add Routine Slot</h2>
              <button onClick={onClose} className="p-1.5 hover:bg-gray-50 dark:hover:bg-[#3d4951]/30 rounded-lg transition-colors">
                <X className="w-5 h-5 text-gray-400 dark:text-[#b1a7a6]" />
              </button>
            </div>
            ...
            {loadingData && (
              <div className="flex flex-col items-center justify-center py-12 gap-3">
                ...
                <p className="text-sm text-gray-400 dark:text-[#b1a7a6]">Loading courses & rooms...</p>
              </div>
            )}
            ...
            {fetchError && (
              <div className="flex flex-col items-center py-8 gap-3 text-center">
                ...
                <button
                  onClick={fetchData}
                  className="px-4 py-2 text-sm bg-indigo-100 dark:bg-red-600 text-white rounded-lg hover:opacity-90 transition"
                >
                  Retry
                </button>
              </div>
            ...
            {!loadingData && !fetchError && (
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                {/* Info */}
                <div className="text-xs text-gray-400 dark:text-[#b1a7a6]/70 bg-gray-50 dark:bg-[#0b090a] rounded-lg p-3 border border-gray-200 dark:border-[#3d4951]">
                  Adding to: <span className="font-medium text-gray-700 dark:text-white">Term {term}</span>
                  &nbsp;•&nbsp;Session: <span className="font-medium text-gray-700 dark:text-white">{session}</span>
                  &nbsp;•&nbsp;Section: <span className="font-medium text-gray-700 dark:text-white">{section}</span>
                </div>
