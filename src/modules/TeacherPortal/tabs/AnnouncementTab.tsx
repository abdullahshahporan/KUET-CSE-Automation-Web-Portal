  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h3 className="font-semibold text-gray-900 dark:text-white">My Announcements</h3>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-white dark:bg-[#161a1d] rounded-xl border border-gray-200 dark:border-[#3d4951]/50 p-6 space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-400 dark:text-[#b1a7a6] mb-1">Title *</label>
          ...
          <div>
            <label className="block text-xs font-medium text-gray-400 dark:text-[#b1a7a6] mb-1">Content *</label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs font-medium text-gray-400 dark:text-[#b1a7a6] mb-1">Type *</label>
            ...
            <div>
              <label className="block text-xs font-medium text-gray-400 dark:text-[#b1a7a6] mb-1">Priority</label>
              <select value={form.priority} onChange={e => updateField('priority', e.target.value)} className={inputClass}>
                {PRIORITIES.map(p => <option key={p} value={p}>{p.charAt(0).toUpperCase() + p.slice(1)}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-400 dark:text-[#b1a7a6] mb-1">Scheduled Date</label>
          ...
          <div className="flex gap-3 justify-end">
            ...
            <button type="submit" disabled={saving || !form.title || !form.content} className="px-4 py-2 bg-gray-600 dark:bg-red-600 text-white rounded-lg text-sm font-medium disabled:opacity-50 flex items-center gap-2">
      ...
      {loading ? (
        ...
        <div className="bg-white dark:bg-[#161a1d] rounded-xl border border-gray-200 dark:border-[#3d4951]/50 p-8 text-center">
          ...
          <p className="text-gray-400 dark:text-[#b1a7a6]">No announcements yet. Create one above.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {announcements.map((a) => (
            <div key={a.id} className="bg-white dark:bg-[#161a1d] rounded-xl border border-gray-200 dark:border-[#3d4951]/50 p-4">
              <div className="flex justify-between items-start">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs px-2 py-0.5 rounded-full bg-gray-50 dark:bg-[#0b090a] text-gray-700 dark:text-[#b1a7a6]">
                      {typeLabels[a.type] || a.type}
                    </span>
                  </div>
                  <p className="text-sm text-gray-500 dark:text-[#b1a7a6]">{a.content}</p>
