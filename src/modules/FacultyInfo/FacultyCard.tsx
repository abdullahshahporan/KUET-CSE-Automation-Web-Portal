...
const getDesignationColor = (designation: TeacherDesignation) => {
  switch (designation) {
    case 'PROFESSOR': return 'bg-red-100 text-red-700 border border-red-200';
    case 'ASSOCIATE_PROFESSOR': return 'bg-blue-100 text-blue-700 border border-blue-200';
    case 'ASSISTANT_PROFESSOR': return 'bg-emerald-100 text-emerald-700 border border-emerald-200';
    case 'LECTURER': return 'bg-amber-100 text-amber-700 border border-amber-200';
...
export default function FacultyCard({ teacher, index, onUpdate, onToggleLeave, onCopyPassword, onDelete }: FacultyCardProps) {
    ...
    ? createPortal(
        <AnimatePresence>
          <motion.div
            ...
            className="w-48 bg-white dark:bg-[#161a1d] border border-gray-200 dark:border-[#3d4951] rounded-xl shadow-xl overflow-hidden"
          >
            <button
              onClick={() => { closeMenu(); onUpdate(teacher); }}
              className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 dark:text-[#f5f3f4] hover:bg-gray-50 dark:hover:bg-[#0b090a] transition-colors"
            >
              <Edit className="w-4 h-4 text-indigo-500 dark:text-red-600" />
              Update Profile
            </button>
            <div className="border-t border-gray-200/50 dark:border-[#3d4951]/50" />
            <button
            ...
            <button
              ...
              className="w-full flex items-center gap-3 px-4 py-3 text-sm text-gray-700 dark:text-[#f5f3f4] hover:bg-gray-50 dark:hover:bg-[#0b090a] transition-colors"
            >
              <Copy className="w-4 h-4 text-indigo-500 dark:text-[#d3d3d3]" />
              Copy Password
            </button>
            <div className="border-t border-gray-200/50 dark:border-[#3d4951]/50" />
            <button
  ...
  return (
    ...
    >
      <SpotlightCard className={`rounded-xl p-5 border h-full ${teacher.is_on_leave ? 'border-amber-300' : 'border-gray-200'}`}>
        <div className="flex items-start gap-4">
          <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white text-xl font-bold ${teacher.is_on_leave ? 'bg-amber-400 opacity-70' : teacher.designation === 'PROFESSOR' ? 'bg-red-500' : teacher.designation === 'ASSOCIATE_PROFESSOR' ? 'bg-blue-500' : teacher.designation === 'ASSISTANT_PROFESSOR' ? 'bg-emerald-500' : 'bg-amber-500'}`}>
            {teacher.full_name.split(' ').map(n => n[0]).join('').slice(0, 2)}
          </div>
          <div className="flex-1 min-w-0">
            <h3 className={`font-semibold ${teacher.is_on_leave ? 'text-gray-400 dark:text-[#b1a7a6]' : 'text-gray-700 dark:text-white'}`}>{teacher.full_name}</h3>
          ...
          <div className="relative">
            <button
              ref={buttonRef}
              onClick={toggleMenu}
              className="p-1.5 rounded-lg text-gray-400 dark:text-[#b1a7a6]/70 hover:text-gray-700 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-[#3d4951]/30 transition-colors"
            >
        ...
        <div className="mt-4 space-y-2 text-sm">
          <div className="flex items-center gap-2 text-gray-400 dark:text-[#b1a7a6]">
