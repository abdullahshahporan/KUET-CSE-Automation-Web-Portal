...
export default function ResultPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div>
          <h1 className="text-2xl font-bold text-gray-700 dark:text-white">Result Management</h1>
          <p className="text-gray-400 dark:text-[#b1a7a6] mt-1">Enter and manage student results</p>
        </div>
        <div className="flex items-center gap-3">
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="px-4 py-2 rounded-full bg-gray-50 dark:bg-[#161a1d] border border-gray-200 dark:border-[#3d4951] text-gray-700 dark:text-[#d3d3d3] hover:border-indigo-400 dark:hover:border-red-400/50 flex items-center gap-2 transition-colors"
          >
            <Download className="w-4 h-4" />
            Export
          </motion.button>
          <div className="flex bg-gray-50 dark:bg-[#161a1d] border border-gray-200 dark:border-[#3d4951] rounded-full p-1">
            <button
              onClick={() => setViewMode('input')}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                viewMode === 'input' 
                  ? 'bg-indigo-100 text-white shadow-lg shadow-[#D9A299]/25' 
                  : 'text-gray-400 dark:text-[#b1a7a6] hover:text-gray-700 dark:hover:text-white'
              }`}
            >
              Input Results
            </button>
      ...
      >
        <div className="relative flex-1 max-w-md">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400 dark:text-[#b1a7a6]/70" />
          <input
            type="text"
            placeholder="Search by name or roll..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-10 pr-4 py-2 border border-gray-200 dark:border-[#3d4951] rounded-lg bg-white dark:bg-[#161a1d] text-gray-700 dark:text-white placeholder:text-gray-400 dark:placeholder:text-white/40 focus:border-indigo-400 dark:focus:border-red-400 focus:outline-none focus:ring-1 focus:ring-indigo-300 dark:focus:ring-red-400"
          />
        </div>
        <select
      ...
      >
        <SpotlightCard spotlightColor="rgba(217, 162, 153, 0.2)">
          <div className="p-4 border-b border-gray-200 dark:border-[#3d4951] flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-gray-700 dark:text-white">
                {dummyCourses.find(c => c.code === selectedCourse)?.title}
              </h3>
              <p className="text-sm text-gray-400 dark:text-[#b1a7a6]">{selectedCourse} - Section {selectedSection}</p>
            </div>
            <span className="text-xs text-gray-400 dark:text-[#b1a7a6]/70 px-3 py-1 rounded-full bg-gray-50 dark:bg-[#0b090a] border border-gray-200 dark:border-[#3d4951]">
              {filteredStudents.length} students
            </span>
          </div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-[#0b090a]/50 border-b border-gray-200 dark:border-[#3d4951]">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">Roll</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">Name</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">CT-1 (20)</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">CT-2 (20)</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">CT-3 (20)</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">Attend. (10)</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">Assign. (5)</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">Total</th>
                  <th className="px-4 py-3 text-center text-xs font-semibold text-gray-700 dark:text-[#b1a7a6] uppercase">Grade</th>
                </tr>
              </thead>
                ...
                {filteredStudents.map((student, index) => {
                  ...
                  return (
                    <motion.tr 
                      ...
                      className="hover:bg-gray-50 dark:hover:bg-red-600/5 transition-colors"
                    >
                      <td className="px-4 py-3 font-medium text-gray-700 dark:text-white">{student.roll}</td>
                      <td className="px-4 py-3 text-gray-400 dark:text-[#d3d3d3]">{student.name}</td>
                      <td className="px-4 py-3 text-center">
                        {viewMode === 'input' ? (
                          <input
                            ...
                            className="w-16 px-2 py-1 text-center border border-gray-200 dark:border-[#3d4951] rounded bg-white dark:bg-[#0b090a] text-gray-700 dark:text-white focus:border-indigo-400 dark:focus:border-red-400 focus:outline-none"
                          />
                        ) : (
                          <span className="text-gray-700 dark:text-[#f5f3f4]">{getResultValue(student.id, 'ct1')}</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
                        {viewMode === 'input' ? (
                          ...
                          <span className="text-gray-700 dark:text-[#f5f3f4]">{getResultValue(student.id, 'ct2')}</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
                        {viewMode === 'input' ? (
                          ...
                          <span className="text-gray-700 dark:text-[#f5f3f4]">{getResultValue(student.id, 'ct3')}</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
                        {viewMode === 'input' ? (
                          ...
                          <span className="text-gray-700 dark:text-[#f5f3f4]">{getResultValue(student.id, 'attendance')}</span>
                        )}
                      </td>
                      <td className="px-4 py-3 text-center">
                        {viewMode === 'input' ? (
                          ...
                          <span className="text-gray-700 dark:text-[#f5f3f4]">{getResultValue(student.id, 'assignment')}</span>
                        )}
                      </td>
          ...
          {viewMode === 'input' && (
            <div className="p-4 border-t border-gray-200 dark:border-[#3d4951] flex justify-end gap-3">
              <motion.button 
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="px-5 py-2.5 rounded-full border border-gray-200 dark:border-[#3d4951] text-gray-700 dark:text-[#d3d3d3] hover:bg-gray-50 dark:hover:bg-[#0b090a] transition-colors"
              >
                Reset
              </motion.button>
              <motion.button 
